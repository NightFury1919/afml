...
- **Real-machine confirmation still needed.** All 41 tests pass in the
  sandbox (Python 3.12.3 / pandas 3.0.2 / numpy 2.4.4); the TDD-header
  comment block in both `.py` files still needs to be refreshed with real
  `mlfinlab`-environment numbers (Python 3.10.20 / pandas 1.5.3 / numpy
  1.23.5) once run there, matching the standard convention.
...
- A handful of `RuntimeWarning: invalid value encountered in divide`
  warnings appear during Part C — these come from `pandas.Series.autocorr()`
  hitting a zero-variance window (a bar with fewer than 2 distinct signed-flow
  values) and correctly returning NaN. Expected, not a bug; worth confirming
  they still appear (and are still harmless) on the real machine.
